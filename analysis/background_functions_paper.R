...
get_conn_pValues <- function(connMatrix, seeds, sOrder, type, th){
  ...
  # one-sample t-test for all ROI-ROI connections across subjects
   if (type == 'greater') {
       connPVals <- apply(connMatrix, c(1,2), function(x) t.test(x, mu = fisherz(th), alternative = type)$p.value)
   } else if (type == 'two.sided') {  #difference between conditions, so test against 0
       connPVals <- apply(connMatrix, c(1,2), function(x) t.test(x, mu = 0, alternative = type)$p.value)
   }
  diag(connPVals) <- 1
  ...
  connPVals$seeds <-seeds  #rows = seeds
  connPVals$seeds <- as.factor(connPVals$seeds) #assign and reorder factor levels to group by network
  ...
  melted_connPVals$value <- melted_connPAdjust #replace original p values with FDR-adjusted ones
  
  # add significance asterix for overlay on connection matrix:
  melted_connPVals$sig <- ''
  melted_connPVals$sig[as.numeric(melted_connPVals$value) < 0.05] <- '*'
...
plot_meanconn <- function(connMatrix, melted_connPVals, seeds, sOrder, title, type, th){
  ...
  #convert from z to r for visualization
  ...
  meanconn$seeds <-seeds #rows = seeds
  meanconn$seeds <- as.factor(meanconn$seeds) #assign and reorder factor levels to group by network
  ...
  # merge mean r values and p values:
  melted_meanconn <- merge(melted_meanconn, melted_connPVals, by=c('seeds','variable'), sort=FALSE, suffixes = c('r','p'))
  
  # now r values are same format as p values, set non-sig correlations to NA
  melted_meanconn$valuer[melted_meanconn$valuep >= 0.05] <- NA
  ...
  p <- ggplot(data = melted_meanconn, aes(x=seeds, y=variable, fill=valuer)) + 
