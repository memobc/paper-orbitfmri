...
# b) compare PM and AT seeds to ANG/PREC by feature
features <- feature_connections(ColorPrecisionPPI, ScenePrecisionPPI, seeds)
sum <- features %>% group_by(SeedNetwork, Feature) %>%
                     summarise(Conn = mean(Connectivity), SE.Conn = se(Connectivity))
print(kable(sum))

# Anova:
pander(ezANOVA(data = features, dv = Connectivity, wid = SubID, 
        detailed = TRUE, within = .(SeedNetwork,Feature)))

# t-test for PM (color vs scene):
pander(t.test(Connectivity ~ Feature, data=features[features$SeedNetwork == 'PM',], paired=TRUE))
# t-test for AT (color vs scene):
pander(t.test(Connectivity ~ Feature, data=features[features$SeedNetwork == 'AT',], paired=TRUE))
